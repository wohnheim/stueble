# TODO: delete file since already handled in websocket
import json
...
def get_open_messages(cursor, session_id: int):
    """
    Retrieve all unreceived messages for a valid and still open sid

    Parameters:
        cursor: Database cursor object.
    """
    SELECT * FROM 
    (SELECT * FROM websockets_affected WHERE session_id = %s) AS affected_messages
    LEFT JOIN websocket_messages AS ws ON message_id = ws.id
    

    result = db.read_table(cursor=cursor,
                  table_name='websockets_affected',
                  keywords=["sid"],
